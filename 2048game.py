...
def draw(window, tiles, game_over=False):
    ...
    if game_over:
        game_over_text = FONT.render("Game Over", 1, FONT_COLOR)
        window.blit(game_over_text, (WIDTH / 2 - game_over_text.get_width() / 2, HEIGHT / 2 - game_over_text.get_height() / 2))
    else:
        welcome_text = FONT.render("Press Arrow Keys to Move", 1, FONT_COLOR)
        window.blit(welcome_text, (WIDTH / 2 - welcome_text.get_width() / 2, HEIGHT / 4 - welcome_text.get_height() / 2))
